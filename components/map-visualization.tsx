...
export function MapVisualization({
          ...
          complete: () => {
            try {
              ...
              if (dataLayers.shippingLanes) {
                addShippingLanes(Cesium, viewer, parseInt(selectedYear), selectedMonth === "all" ? undefined : parseInt(selectedMonth))
              }
  ...
  // Fetch shipping coordinates from the API
  async function fetchShippingCoordinates(year: number, month?: number): Promise<number[]> {
    try {
      // Construct the API URL based on the presence of year and month
      let url = "http://localhost:5000/data/shipping/";
      if (year && month) {
        url += `${month}/${year}`;
      } else if (year) {
        url += `${year}`;
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch shipping coordinates: ${response.statusText}`);
      }
  
      const data = await response.json();
      if (data.status !== "1" || !data.lane) {
        throw new Error("Invalid response from the API");
      }
  
      // Assuming the API returns a flattened array of coordinates
      return data.lane;
    } catch (error) {
      console.error("Error fetching shipping coordinates:", error);
      return [];
    }
  }

  async function addShippingLanes(Cesium: any, viewer: any, year: number, month?: number) {
    try {
      // Fetch custom shipping lane coordinates from the API
      const customShippingCoords = await fetchShippingCoordinates(year, month);
  
      if (customShippingCoords.length === 0) {
        console.warn("No custom shipping coordinates available.");
        return;
      }

      // Filter out segments where the distance between two points is greater than 3 degrees
    const filteredCoords: number[] = [];
    for (let i = 0; i < customShippingCoords.length - 2; i += 2) {
      const lon1 = customShippingCoords[i];
      const lat1 = customShippingCoords[i + 1];
      const lon2 = customShippingCoords[i + 2];
      const lat2 = customShippingCoords[i + 3];

      const distance = Math.sqrt(Math.pow(lon2 - lon1, 2) + Math.pow(lat2 - lat1, 2));
      if (distance <= 3) {
        if (distance <= 3) {
          // Add only the valid segment's starting point and end point
          if (filteredCoords.length === 0) {
            filteredCoords.push(lon1, lat1);
          }
          filteredCoords.push(lon2, lat2);
        }
      }
    }

    if (filteredCoords.length === 0) {
      console.warn("No valid segments for the shipping lane.");
      return;
    }
  
      // Add your custom shipping lane with a thicker line
      viewer.entities.add({
        polyline: {
          positions: Cesium.Cartesian3.fromDegreesArray(filteredCoords),
          width: 5, // Thicker line
          material: new Cesium.PolylineDashMaterialProperty({
            color: Cesium.Color.ORANGE, // Keep orange color as requested
            dashLength: 16.0, // Different dash pattern
          }),
        },
        properties: {
          type: "shipping-lane",
          laneType: "Custom",
          id: "custom",
          name: `Shipping Lane ${year || ""}${month ? ` - Month ${month}` : ""}`,
        },
      });
  
      // Add a label for the custom shipping lane
      const midIndex = Math.floor(filteredCoords.length / 4) * 2; // Get a point near the middle
      viewer.entities.add({
        position: Cesium.Cartesian3.fromDegrees(
          filteredCoords[midIndex],
          filteredCoords[midIndex + 1],
          100000, // Height above surface
        ),
        label: {
          text: `Shipping Lane ${year || ""}${month ? ` - Month ${month}` : ""}`,
    ...
    <div className="flex-1 relative bg-migratewatch-dark">
      ...
      <div className="absolute bottom-4 left-4 z-10 bg-migratewatch-panel/80 p-3 rounded-md">
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center space-x-2">
            <div className="w-4 h-0.5 bg-migratewatch-orange" style={{ height: "3px" }}></div>
            <span className="text-xs">Your Custom Shipping Lane</span>
          </div>
          <div className="flex items-center space-x-2">
