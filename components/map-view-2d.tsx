...
export function MapView2D({
  ...
  }, [scriptLoaded, dataLayers, selectedSpecies, selectedYear, selectedMonth, onError, mapStyle])
  ...
  function addShippingLanes(map: any) {
    try {
      // Standard shipping lane
      const standardShippingPath = [
        [40.7, -74.0],
        [42.0, -60.0],
        [45.0, -40.0],
        [48.0, -20.0],
      ...
      // Create the standard shipping lane
      const standardShippingLane = window.L.polyline(standardShippingPath, {
        color: "#ff9e00",
        weight: 3,
        opacity: 0.8,
        dashArray: "8, 4",
      ...
      standardShippingLane.bindTooltip("Standard Shipping Lane", {
        permanent: false,
        direction: "top",
        className: "shipping-lane-tooltip",
      })

      // Your custom shipping lane coordinates
      const customShippingPath = [
        [-8.1373795949999703, -34.857596796999928],
        [-8.0702548089999482, -34.674369623999951],
        [-8.0702491859999554, -34.674368303999984],
        [16.957174956000074, -25.630149081999946],
        [17.286788839000053, -25.483361960999957],
        [17.616515286000038, -25.337770409999962],
        [17.946354715000041, -25.190650972999947],
        [18.377015454000059, -24.99966827399993],
        [18.807669188000034, -24.806104355999935],
        [19.668971265000039, -24.411254697999937],
        [20.099627421000037, -24.211280570999975],
        [20.530292196000062, -24.013673022999967],
        [20.960969498000054, -23.814493525999978],
        [21.176314066000032, -23.714317912999945],
      ]

      // Convert coordinates to Leaflet format [lat, lng]
      const leafletCustomCoords = customShippingPath.map((coord) => [coord[0], coord[1]])

      // Create the custom shipping lane with a thicker line
      const customShippingLane = window.L.polyline(leafletCustomCoords, {
        color: "#ff9e00", // Keep orange color as requested
        weight: 5, // Make it thicker
        opacity: 1.0,
        dashArray: "10, 5", // Different dash pattern
        className: "shipping-lane shipping-element",
      }).addTo(map)

      // Add tooltip with information
      customShippingLane.bindTooltip("Your Custom Shipping Lane", {
        permanent: false,
        direction: "top",
        className: "shipping-lane-tooltip",
      })

      // Add click handler for the custom shipping lane
      customShippingLane.on("click", () => {
        // Show info about this shipping lane
        const bounds = customShippingLane.getBounds()
        ...
        // Display info card
        setActiveInfoCard({
          title: "Custom Shipping Lane Analysis",
      ...
      const midIndex = Math.floor(leafletCustomCoords.length / 2)
      const midPoint = leafletCustomCoords[midIndex]
      ...
      // Keep the ship icons and animation from the original function
